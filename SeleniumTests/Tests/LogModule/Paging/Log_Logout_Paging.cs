    ...
    public class Log_Logout_Paging
    {
        ...
        public void Log_Logout_TestPagingFirstButtonAndVerify()
        {
            try
            {
                ...
                LogStep("🔍 Capturing original table first row text");
                ...
                Assert.IsTrue(!lastDisabled, "❌ 'Last' button is disabled — possibly only one page.");
                ...
                LogStep("Clicking 'First' page button");
                ...
                Assert.IsTrue(!firstDisabled, "❌ 'First' button is disabled after clicking 'Last'.");
                ...
                LogStep("⏳ Waiting for table to return to original first row");
                ...
                var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                Assert.IsTrue(tableReturned, "❌ Table did not return to original state after clicking 'First'.");
                LogStep("✅ Table returned to original after clicking 'First' button.");
            }
            catch (Exception ex)
            {
                _lastScreenshotPath = Path.Combine(Path.GetTempPath(), $"Log_Logout_Page_{DateTime.Now:yyyyMMdd_HHmmss}.png");
                var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
                File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
                Assert.Fail("❌ Test failed due to unexpected exception: " + ex.Message);
            }
        }
        ...
        public void Log_Logout_TestItemsPerPageVerify(string pageSizeValue)
        {
            ...
            LogStep("🔍 Capturing original table content and row count");
            var tableElement = _wait.Until(ExpectedConditions.ElementIsVisible(By.XPath(tableXPath)));
            string beforeHtml = tableElement.GetAttribute("innerHTML");
            ...
            LogStep("📋 Validating page size option in dropdown");
            ...
            var screenshot = ((ITakesScreenshot)_driver).GetScreenshot();
            File.WriteAllBytes(_lastScreenshotPath, screenshot.AsByteArray);
            Assert.IsTrue(tableUpdated, "❌ Table did not update after selecting new page size.");
            LogStep("✅ Table updated successfully after changing page size.");
        }
